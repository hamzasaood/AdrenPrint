...
{
    // Show main DTF product page
    public function index()
    {
        $images = DtfImage::where('type', 'dtf-transfer-by-size')->get();
        $colors = DtfColor::where('type', 'dtf-transfer-by-size')->where('is_active',1)->orderBy('id')->get();
        $sizes  = DtfSize::where('type', 'dtf-transfer-by-size')->where('is_active',1)->orderBy('id')->get();
